...
public class AltoMando_Section02_Controller : MonoBehaviour, IRoom, IMultipleRoom
{
    ...
    {
        /*
        doorController_A1.DisableAutoDoorOpener();
        doorController_A2.DisableAutoDoorOpener();
        doorController_A3.DisableAutoDoorOpener();
        if (gameState.FourDoors[0].isCompleted) { doorController_A1.InstaOpen(); }
        if (gameState.FourDoors[1].isCompleted) { doorController_A2.InstaOpen(); }
